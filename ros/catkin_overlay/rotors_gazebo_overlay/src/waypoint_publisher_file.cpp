...
int main(int argc, char** argv) {
  ...
  //ros::Duration(10).sleep();//si esta en 0 o se quita el dron no lee los waypoints, es importante

  ROS_INFO("Waiting for 10 seconds...");
  ros::Time startTime = ros::Time::now();

  for (int i = 0; i < 10; i++) {
    ros::Duration(1).sleep(); // Dormir durante 1 segundo
    ROS_INFO("Elapsed time: %.2f seconds", (ros::Time::now() - startTime).toSec()); // Imprimir el tiempo transcurrido
  }
  ...
  for (size_t i = 0; i < waypoints.size(); ++i) {
    ...
    //ROS_INFO("Number of waipoint %d", (int) i); linea debug
